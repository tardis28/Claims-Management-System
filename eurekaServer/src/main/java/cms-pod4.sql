...
drop table if exists `claim`.`member_claim`;
create table if not exists `claim`.`member_claim`(
...
insert into user (userid,password,username) values ('mogambo','root@6','admin');
